...
class PraktykaApp(QtGui.QMainWindow, ui_design.Ui_MainWindow):
    ...
    def save_file(self):
        ...
        if name and len(name)>0:
            ...
            print("sdfsdf", name)
            file1 = open( "_AnalysisSymbol.txt", 'w' )

            self.write_loop(file1, 1)

            file2 = open(name + "_dti.txt", 'w' )

            self.write_loop(file2, 2)

            file3 = open(name + "_Ti.txt", 'w' )

            self.write_loop(file3, 3)

            file4 = open(name + "_NewSymbols.txt", 'w' )
    ...
    def build(self):
        ...
        print("AFAF",self.getFromLineEdit())

        self.N = self.getFromLineEdit()

        self.all_calculations.setIntervals(self.getFromLineEdit())
    ...
    def fill_table(self, getTableNumber):
        ...
        if tableNumber == 1:
            ...
            self.P_pad3 = sorted(self.all_calculations.P_pad3, reverse = True)
    ...
    def buildGrahTab3(self):
        ...
        if self.radioButton_4.isChecked():
            ...
            iteration_y = self.p_i_schtrich

            newArrayY = []

            for i in range(len(self.p_i_schtrich)):
